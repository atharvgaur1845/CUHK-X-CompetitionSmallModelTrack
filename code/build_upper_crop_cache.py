...
OUT_ROOT = ROOT / "cache" / "crop_upper"
...
UPPER_FRACTION = 0.62   # head -> hips; covers hands in seated and standing manipulation
...
def crop_window(model, ir_frames: list[Path], width: int, height: int):
    ...
    # UPPER-BODY variant. Measured motivation: 86.4% of champion errors are
    # object->object, concentrated in Read_documents/Turn_pages (49),
    # tableware/pour/stir/peel (~60), Drink/Eat (34), Sweep/Mop (31). Every one of
    # those pairs shares a posture and differs only in the small held object and the
    # fine hand trajectory -- which occupy ~15x15 px of a 128x128 whole-body crop.
    # Keeping the top UPPER_FRACTION of the box magnifies hands ~3x at the same cost.
    # This is a COMPLEMENTARY member, not a replacement: Sweep/Mop is discriminated by
    # the tool head at floor level, which this crop deliberately cuts away, so the
    # full-body member must stay in the bag to cover it.
    y1 = y0 + (y1 - y0) * UPPER_FRACTION
