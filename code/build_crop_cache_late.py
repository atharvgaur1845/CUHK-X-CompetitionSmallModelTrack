...
LATE_FRACTION = 0.55   # keep the final 55% of the clip


def endpoint_uniform(items: list, count: int) -> list:
    """Evenly spaced over the FINAL LATE_FRACTION of the clip, both endpoints included.

    A participant reported on the competition forum (topic 735601, 2026-08-17) that
    clips contain several activities and the label names one of them -- "writing might
    contain writing, turning pages, and reading a document", and "lying down containing
    walking, sitting down, and then lying down". The second example says the label is
    the activity the clip ENDS on, not the one it spends the most frames in.

    That matches our own measured error structure: 86.4% of champion errors are
    object->object and the largest single cluster is exactly Read_documents <-> Turn_pages
    (49 errors), the pair that participant names. Sampling uniformly over the whole clip
    and global-average-pooling then averages the labelled activity together with the
    lead-in activities, which are themselves valid classes.
    """
    if not items:
        return []
    if len(items) == 1:
        return items * count
    start = int(len(items) * (1.0 - LATE_FRACTION))
    tail = items[start:] or items
    if len(tail) == 1:
        return tail * count
    idx = np.linspace(0, len(tail) - 1, count).round().astype(int)
    return [tail[i] for i in idx]
